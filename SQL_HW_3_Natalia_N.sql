...
--После того как создадите свои таблицы в ДЗ SQL_DDL, на этих же созданных своих таблицах делайте  SQL HomeWork 3. Joins

--Подключится к 
--Host: 159.69.151.133
--Port: 5056
--DB: подключение из таблицы доступов
--User: подключение из таблицы доступов
--Pass: 123
 ...
 select min(s.monthly_salary) as min_salary_QA_engineer 
 from salary s
 left join employee_salary es 
 on s.id = es.salary_id
 left join employees e 
 ...
 select max(s.monthly_salary) as max_salary_QA_engineer
 from salary s
 left join employee_salary es 
 on s.id = es.salary_id
 left join employees e 
 ...
 select count (role_name) as count_QA_engineer
 from roles r
 join roles_employee re 
 on r.id = role_id
